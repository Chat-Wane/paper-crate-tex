...
\item [Operational transformation] (OT) allows building both centralized and
  ...
  Whatever the tradeoff, OT's integration algorithms rely on concurrency
...
\item [Conflict-free replicated data types~\cite{shapiro2011comprehensive,
  ...
  CRDTs propose interesting tradeoff since they can balance complexities
...
\item [Variable-size identifiers~\cite{nedelec2013lseq, preguica2009commutative,
    weiss2009logoot}] CRDTs such as Logoot~\cite{weiss2009logoot} truly destroy
  elements targeted by removals.  Since removals truly erase information from
  the structure, these approaches require a local state vector compacting their
  history, hence an additional $\mathcal{O}(W)$ on space complexity. It is worth
  noting that, contrarily to OT, the vector is kept local. Variable-size
  ...
  growth is linear compared to the number of insertions $I$ in the sequence,
