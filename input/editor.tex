...
% \noindent For instance, in Figure~\ref{fig:timeline}, the vectors are
% represented as a list of triples
...
%The anti-entropy
%protocol~\cite{demers1987epidemic} reduces the information differences between
...
% For instance, in Figure~\ref{fig:timeline}, when the editor $u_2$ receives the
% second operation of $u_1$, it notices that the first operation of the latter is
% missing. Thus, it can actively ask to some editors for the missing operation.
...
% Strong eventual consistency states that a system is correct if replicas
% integrating a same set of operations converge to an equivalent
% state~\cite{shapiro2011comprehensive}.
...
% Being sublinear, the communication complexity is practicable. 
