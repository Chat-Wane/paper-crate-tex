...
is [$31.63.127$], etc. 
Overaal, the space complexity of the tree is $\mathcal{O}(I^2)$ too.
...
instance, considering two elements associated to the following paths: [$4.2$]
and [$4.2.1$]. If one deletes the first element, the path is kept since the path
labeled by 4 then 2 is still required for the second element. Then, if one
...
level $l$ at which it is performed: $\mathcal{O}(\log 2^l)$. The repeated binary
searches lead to an upper bound of
