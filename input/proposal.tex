...
\begin{figure}
  ...
  [Left-to-right editing behavior with \LSEQ.]
  {\input{input/lseqtreeexampleA.tex}}
  \subfloat[Right-to-left case with \LSEQ.]
  [Right-to-left editing behavior with \LSEQ.]
...
balanced and \TODO{performance are more efficient proportionnally}. Also, the
...
obviously very complex. \TODO{Moar about difficulty of humans?}
...
to all variable-size identifier allocators (\REF). However, since \LSEQ uses a
tree structure factorizing common parts of identifiers, the overall space
complexity is not the sum of identifiers but only $\mathcal{O}(I\log I)$.
...
designs settle each other's deficiency.
