...
\newcommand*{\comment}[1]{$\rhd$ #1}
  ...
  \Statex
    ...
    \State \textbf{let} $depth,\,\_ \leftarrow \textsc{getDepthInterval}(p,\,q)$;
    \State \textbf{return} $h(depth)(p,\,q)$; \hfill \comment{Defers the call}
    \EndFunction
    \Statex
    \Function{endEditing}{$p,\,q \in \mathcal{P}$}
    $\rightarrow \mathcal{P}$
    \State \textbf{let} $depth,\,interval \leftarrow \textsc{getDepthInterval}(p,q);$
    ...
    \EndFunction
    \Statex 
    \Function{getDepthInterval}{$p,\,q \in \mathcal{P}$} $\rightarrow \mathbb{N} \times \mathbb{N}$ \hfill \comment{Which level has enough space for 1 path}
