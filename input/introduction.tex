...
\item to measure the overall performances of a real working system, we
  built CRATE: a real-time decentralized editor running in web
  browsers~\cite{nedelec2016crate}. Compared to Google doc, CRATE
  enable real-time editing whatever the number of participants. to
  better preserve privacy of users, CRATE ensures that shared
  documents only exist within browsers participating to the real-time
  editing session. In this paper, we describe the complete
  architecture of CRATE with underlying algorithms and analyse the
  complexity of each component.
\item It provides experiments characterizing \LSEQ's behavior that
  validates our analysis and compares with state-of-the-art
  approaches. Results show that \LSEQ achieves an original tradeoff
  between communication, time and space complexity on a real working
  system. In the Grid'5000 testbed, we launched CRATE editing sessions
  involving up till 600 connected browsers. The resulting documents
  reach millions of characters. It validates the scalability of a full
