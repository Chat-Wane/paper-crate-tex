...
\newcommand\ADDY{-10};
  ...
  %% node to node
  \small
  \draw[dashed,thick] (0pt,0pt) -- node[anchor=south east]{0} (-75pt,-30pt);
  ...
  \draw[dashed,thick] (0pt,0pt) -- node[anchor=south west]{9} ( 75pt,-30pt);
...
%  \draw[->] (  0pt,3 * \Y pt) -- (  0pt, \ADDY + 3 * \Y pt);
  ...
  \draw[fill=black] (-75pt, -30pt) circle (1pt); 
...
%  node{\textbf{?}} +(-4pt,-4pt) rectangle +(4pt,4pt) ; %% ?
  ...
  \begin{scope}[shift={(45pt, 2.5*\Y pt)}]    
     \draw[fill=white](0, 1 + \ADDY pt)node{\textbf{E}} +(-4pt, -4pt) rectangle +(4pt, 4pt);
     \scriptsize
     \draw (4pt, 1+\ADDY pt)node[anchor=west]{element};
